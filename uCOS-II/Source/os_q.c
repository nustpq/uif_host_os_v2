...
/*  PQ added this function
*********************************************************************************************************
*                                      GET MESSAGE FROM QUEUE, BUT not delete it from the ququq
*
* Description: This function checks the queue to see if a message is available, and read it only.  
*              Unlike OSQAccept(), it does not delete the read message if it available.
*
* Arguments  : pevent        is a pointer to the event control block
*
*              perr          is a pointer to where an error message will be deposited.  Possible error
